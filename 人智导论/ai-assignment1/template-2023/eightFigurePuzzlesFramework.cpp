...
bool operator==(PUZZLE_NODE a, PUZZLE_NODE b) { return checkObject(a, b); }
...
void openListInsert(vector<PUZZLE_NODE>& openList, PUZZLE_NODE& nextNode) {
    ...
    else if (openList[openList.size() - 1].height + openList[openList.size() - 1].depth
             <= nextNode.height + nextNode.depth)
        openList.push_back(nextNode);
    else {
...
bool operator>(PUZZLE_NODE a, PUZZLE_NODE b) { return a.height + a.depth > b.height + b.depth; }
// 广度优先搜索
...
vector<int> heuristicSearchInformedByIncorrectNum(PUZZLE_NODE initialNode, PUZZLE_NODE objPuzzleNode) {
    ...
    */
    ...
    /*
        Note that line 75 manually implements the overloading of operator >, and the same is true in
       heuristic search 2. The method of using vector to maintain monotonic sequences provided at
       the beginning of the 2023 version of the framework is too inefficient and is not recommended.
    */
    PUZZLE_NODE currentNode, nextNode;
...
int main() {
    ...
    while (setup != -1) {
        ...
        cout << "请输入调试设置(-1:退出; "
                "0:广度优先搜索示例;1:广度优先搜索;2：深度有限搜索;3:启发式搜索1;4:启发式搜索2):"
             << endl;
        cin >> setup;
        int backwardSteps;
